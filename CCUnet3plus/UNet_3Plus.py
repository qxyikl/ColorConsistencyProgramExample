...
import sys
sys.path.append('./CCUnet3plus')
from layers import unetConv2
from init_weights import init_weights
import numpy as np

'''
    UNet 3+/UNet 3+ with deep supervision/UNet 3+ with deep supervision and class-guided module
'''
...
class UNet_3Plus(nn.Module):
    ...
    def __init__(self, in_channels=3, n_classes=3, feature_scale=4, is_deconv=True, is_batchnorm=True): #in_channels=3表示输入的是彩色图
...
class UNet_3Plus_DeepSup_CGM(nn.Module):
    ...
    #将分割结果与分类二维矩阵进行相乘，返回四维乘积结果。(比上一个模型多出来的部分)
...
'''
    UNet 3+ with deep supervision 2(3)layers-W
'''
class UNet_3Plus_DeepSup_FS2W(nn.Module):
    def __init__(self, in_channels=3, n_classes=3, feature_scale=2, is_deconv=True, is_batchnorm=True):
        super(UNet_3Plus_DeepSup_FS2W, self).__init__()
        self.is_deconv = is_deconv
        self.in_channels = in_channels
        self.is_batchnorm = is_batchnorm
        self.feature_scale = feature_scale
        ...
        ## -------------Encoder1--------------
        self.conv1 = unetConv2(self.in_channels, filters[1], self.is_batchnorm)
        self.maxpool1 = nn.MaxPool2d(kernel_size=2)
 
        self.conv2 = unetConv2(filters[1], filters[2], self.is_batchnorm)
        self.maxpool2 = nn.MaxPool2d(kernel_size=2)
 
        self.conv3 = unetConv2(filters[2], filters[3], self.is_batchnorm)
 
        ## -------------Decoder1--------------
        self.CatChannels = filters[1]
        self.CatBlocks = 3
        self.UpChannels = self.CatChannels * self.CatBlocks
        ...
        # h1->320*320, hd2->160*160, Pooling 2 times
        self.h1_PT_hd2 = nn.MaxPool2d(2, 2, ceil_mode=True)
        self.h1_PT_hd2_conv = nn.Conv2d(filters[1], self.CatChannels, 3, padding=1)
        self.h1_PT_hd2_bn = nn.BatchNorm2d(self.CatChannels)
        self.h1_PT_hd2_relu = nn.ReLU(inplace=True)
        ...
        # hd3->80*80, hd2->160*160, Upsample 2 times
        self.hd3_UT_hd2 = nn.Upsample(scale_factor=2, mode='bilinear')  # 14*14
        self.hd3_UT_hd2_conv = nn.Conv2d(filters[3], self.CatChannels, 3, padding=1)
        self.hd3_UT_hd2_bn = nn.BatchNorm2d(self.CatChannels)
        self.hd3_UT_hd2_relu = nn.ReLU(inplace=True)
        ...
        '''stage 1d'''
        # h1->320*320, hd1->320*320, Concatenation
        self.h1_Cat_hd1_conv = nn.Conv2d(filters[1], self.CatChannels, 3, padding=1)
        self.h1_Cat_hd1_bn = nn.BatchNorm2d(self.CatChannels)
        self.h1_Cat_hd1_relu = nn.ReLU(inplace=True)
        ...
        # hd3->80*80, hd1->320*320, Upsample 4 times
        self.hd3_UT_hd1 = nn.Upsample(scale_factor=4, mode='bilinear')  # 14*14
        self.hd3_UT_hd1_conv = nn.Conv2d(filters[3], self.CatChannels, 3, padding=1)
        self.hd3_UT_hd1_bn = nn.BatchNorm2d(self.CatChannels)
        self.hd3_UT_hd1_relu = nn.ReLU(inplace=True)
        ...
        self.outconv1 = nn.Conv2d(self.UpChannels, n_classes, 3, padding=1)
        self.outconv2 = nn.Conv2d(self.UpChannels, n_classes, 3, padding=1)
        self.outconv3 = nn.Conv2d(filters[3], n_classes, 3, padding=1)


        ## -------------Encoder2--------------
        self.conv4 = unetConv2(self.in_channels, filters[1], self.is_batchnorm)
        self.maxpool3 = nn.MaxPool2d(kernel_size=2)
 
        self.conv5 = unetConv2(filters[1], filters[2], self.is_batchnorm)
        self.maxpool4 = nn.MaxPool2d(kernel_size=2)
 
        self.conv6 = unetConv2(filters[2], filters[3], self.is_batchnorm)
        ...
        self.h4_PT_hd5_conv = nn.Conv2d(filters[1], self.CatChannels, 3, padding=1)
        self.h4_PT_hd5_bn = nn.BatchNorm2d(self.CatChannels)
        self.h4_PT_hd5_relu = nn.ReLU(inplace=True)
 
        # h2->160*160, hd2->160*160, Concatenation
        self.h5_Cat_hd5_conv = nn.Conv2d(filters[2], self.CatChannels, 3, padding=1)
        ...
        self.hd6_UT_hd5_conv = nn.Conv2d(filters[3], self.CatChannels, 3, padding=1)
        self.hd6_UT_hd5_bn = nn.BatchNorm2d(self.CatChannels)
        self.hd6_UT_hd5_relu = nn.ReLU(inplace=True)
 
        # fusion(h1_PT_hd2, h2_Cat_hd2, hd3_UT_hd2, hd4_UT_hd2, hd5_UT_hd2)
        self.conv2d_2 = nn.Conv2d(self.UpChannels, self.UpChannels, 3, padding=1)  # 16
        self.bn2d_2 = nn.BatchNorm2d(self.UpChannels)
        self.relu2d_2 = nn.ReLU(inplace=True)
 
        '''stage 1d'''
        # h1->320*320, hd1->320*320, Concatenation
        self.h4_Cat_hd4_conv = nn.Conv2d(filters[1], self.CatChannels, 3, padding=1)
        self.h4_Cat_hd4_bn = nn.BatchNorm2d(self.CatChannels)
        self.h4_Cat_hd4_relu = nn.ReLU(inplace=True)
 
        # hd2->160*160, hd1->320*320, Upsample 2 times
        self.hd5_UT_hd4 = nn.Upsample(scale_factor=2, mode='bilinear')  # 14*14
        self.hd5_UT_hd4_conv = nn.Conv2d(self.UpChannels, self.CatChannels, 3, padding=1)
        self.hd5_UT_hd4_bn = nn.BatchNorm2d(self.CatChannels)
        self.hd5_UT_hd4_relu = nn.ReLU(inplace=True)
 
        # hd3->80*80, hd1->320*320, Upsample 4 times
        self.hd6_UT_hd4 = nn.Upsample(scale_factor=4, mode='bilinear')  # 14*14
        self.hd6_UT_hd4_conv = nn.Conv2d(filters[3], self.CatChannels, 3, padding=1)
        self.hd6_UT_hd4_bn = nn.BatchNorm2d(self.CatChannels)
        self.hd6_UT_hd4_relu = nn.ReLU(inplace=True)
 
        # fusion(h1_Cat_hd1, hd2_UT_hd1, hd3_UT_hd1, hd4_UT_hd1, hd5_UT_hd1)
        self.conv1d_2 = nn.Conv2d(self.UpChannels, self.UpChannels, 3, padding=1)  # 16
        self.bn1d_2 = nn.BatchNorm2d(self.UpChannels)
        self.relu1d_2 = nn.ReLU(inplace=True)
 
        # -------------Bilinear Upsampling--------------     #双线性上采样
        self.upscore5 = nn.Upsample(scale_factor=4,mode='bilinear')
        self.upscore4 = nn.Upsample(scale_factor=2, mode='bilinear')
 
        # DeepSup #深监督：将decoder的最后一层进行conv3*3，bilinear upsampling, sigmoid
        self.outconv4 = nn.Conv2d(self.UpChannels, n_classes, 3, padding=1)
        self.outconv5 = nn.Conv2d(self.UpChannels, n_classes, 3, padding=1)
        self.outconv6 = nn.Conv2d(filters[3], n_classes, 3, padding=1)
    ...
    def forward(self, inputs):
        ...
        h3 = self.maxpool2(h2)
        hd3 = self.conv3(h3)  # h5->20*20*1024
        ...
        ## -------------Encoder-------------
        h4 = self.conv1(inputs)  # h1->320*320*64
 
        h5 = self.maxpool1(h4)
        h5 = self.conv2(h5)  # h2->160*160*128
 
        h6 = self.maxpool2(h5)
        hd6 = self.conv3(h6)  # h5->20*20*1024
 
        ## -------------Decoder------------- 
        #decoder2
        h4_PT_hd5 = self.h4_PT_hd5_relu(self.h4_PT_hd5_bn(self.h4_PT_hd5_conv(self.h4_PT_hd5(h4))))
        h5_Cat_hd5 = self.h5_Cat_hd5_relu(self.h5_Cat_hd5_bn(self.h5_Cat_hd5_conv(h5)))
        hd6_UT_hd5 = self.hd6_UT_hd5_relu(self.hd6_UT_hd5_bn(self.hd6_UT_hd5_conv(self.hd6_UT_hd5(hd6))))
        hd5 = self.relu2d_2(self.bn2d_2(self.conv2d_2(
            torch.cat((h4_PT_hd5, h5_Cat_hd5, hd6_UT_hd5), 1)))) # hd2->160*160*UpChannels
 
        #decoder1
        h4_Cat_hd4 = self.h4_Cat_hd4_relu(self.h4_Cat_hd4_bn(self.h4_Cat_hd4_conv(h4)))
        hd5_UT_hd4 = self.hd5_UT_hd4_relu(self.hd5_UT_hd4_bn(self.hd5_UT_hd4_conv(self.hd5_UT_hd4(hd5))))
        hd6_UT_hd4 = self.hd6_UT_hd4_relu(self.hd6_UT_hd4_bn(self.hd6_UT_hd4_conv(self.hd6_UT_hd4(hd6))))
        hd4 = self.relu1d_2(self.bn1d_2(self.conv1d_2(
            torch.cat((h4_Cat_hd4, hd5_UT_hd4, hd6_UT_hd4), 1)))) # hd1->320*320*UpChannels
 
        #深监督：将每个decoder的最后一层进行conv3*3，bilinear upsampling, sigmoid
        #decoder3的深监督
        d6 = self.outconv3(hd6)
        d6 = self.upscore3(d6) # 64->256
 
        #decoder2的深监督
        d5 = self.outconv2(hd5)
        d5 = self.upscore2(d5) # 128->256
 
        #decoder1的深监督
        d4 = self.outconv1(hd4) # 256  #decoder1不做上采样处理
        return F.sigmoid(d1), F.sigmoid(d6)#将深监督的结果进行sigmoid，做最后处理
